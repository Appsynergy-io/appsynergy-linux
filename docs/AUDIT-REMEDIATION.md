...
1. **Build + publish** — `packages/scripts/build-appsynergy-linux.sh`, then `build-repo.sh` and `publish-repo.sh`, then `verify-repo.sh`.
2. **Re-verify at window time.** `pacman -Si appsynergy-linux` resolves to the intended version under `Required DatabaseRequired`, and nothing has superseded it.
